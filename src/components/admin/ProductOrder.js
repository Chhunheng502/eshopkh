...
function ProductOrder() {
    ...
    const handleAcceptOrder = (id) => {
        ...
        axios.put(`https://eshopkh-api.herokuapp.com/api/orders/update/${id}`)
        .then((response) => {
            console.log('Success:', response);
            setContent(contents.filter(item => item.id !== id));
        })
    ...
    const handleCancelAll = () => {
        ...
        for(const data of contents)
        {
            ...
            {
                axios.delete(`https://eshopkh-api.herokuapp.com/api/orders/delete/${data.id}`)
                .then((response) => {
                    console.log('Success:', response);
                })
                .catch((error) => {
    ...
    const handleAcceptAll = () => {
        ...
        for(const data of contents)
        {
            if(document.getElementById(`order-${data.id}`) !== null && document.getElementById(`order-${data.id}`).checked === true)
            {
                axios.put(`https://eshopkh-api.herokuapp.com/api/orders/update/${data.id}`)
                .then((response) => {
                    console.log('Success:', response);
                })
                .catch((error) => {
